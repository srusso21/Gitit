...
Set-CASMailbox -Identity $upn -ActiveSyncEnabled $True

#gives manager full access
Remove-MailboxPermission -identity $upn -User $Manager.emailaddress -AccessRights FullAccess -InheritanceType all

#hide from the GAL
Set-ADUser $user -Replace @{msExchHideFromAddressLists=$False}
