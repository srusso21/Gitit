...
$ComputersList = Get-ADComputer -Filter 'name -like "ws-*"' -Properties CanonicalName,LastLogonDate | Where-Object -FilterScript {$_.CanonicalName -notmatch 'test' -and $_.CanonicalName -notmatch 'admin' -and $_.CanonicalName -notmatch 'bim9' -and $_.LastLogonDate -notmatch '2019' -and $_.LastLogonDate -match "$date"} | Sort-Object -Property LastLogonDate -Descending | select -ExpandProperty name
...
$remotec = "\\$connected\C$"
Copy-item -Path ".\Content.zip" -Destination "$remotec\temp\" -Force
Copy-item -Path ".\WMVPN.ps1" -Destination "$remotec\temp\" -Force
If((Test-Path "$remotec\temp\Content.zip") -and (Test-Path "$remotec\temp\WMVPN.ps1")){
Write-Host -ForegroundColor Green "Copied files to $connected"
}else{
Write-Host -ForegroundColor Green "$connected failed to copy"
}
}
